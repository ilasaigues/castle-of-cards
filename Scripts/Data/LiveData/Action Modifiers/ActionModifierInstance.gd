...
func is_valid(sourceChar:CharacterInstance,context:ActionContext):
	var valid = true
	for condition in modifier_conditionals:
		if condition is BoolConditionData:
			valid = valid and check_bool_condition(context, condition, sourceChar)
...
func check_bool_condition(context:ActionContext, condition:BoolConditionData, sourceChar:CharacterInstance):
	match condition.condition_type:
		...
		Enums.BoolConditionType.ActorIsPlayer:
			return context.actor.base_data.is_player && context.actor == sourceChar
